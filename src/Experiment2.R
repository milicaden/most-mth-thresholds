...
d = load.data(Folder, "experiment-two")

#Exclude subjects who did multiple versions of the experiment
multiple_id = data.frame(table(d$ProlificID))
names(multiple_id) = c("ProlificID", "number_of_resp")
multiplesubjid = subset(multiple_id, number_of_resp !=146)$ProlificID
d <- subset(d, !(ProlificID %in% multiplesubjid))

# Load the design files
design1 <- read.csv(paste0(Folder, "Experiment2-most-design.csv"), header = TRUE, sep = ",")
design2 <- read.csv(paste0(Folder, "Experiment2-mth-design.csv"), header = TRUE, sep = ",")
...
#png("Experiment2-thresholds.png", width = 160, height = 100, units='mm', res = 300)
ggplot(data=finalthresholds, aes(finalthresholds$xmid)) + 
  geom_histogram(position = "identity", breaks=seq(30, 90, by =10), 
                 alpha = .4)+
  facet_grid(Monotonicity~Quantifier)+
  labs(x="Thresholds in Experiment 2")
#dev.off()
...
#Plot#
#png("Experiment2-borderline.png", width = 160, height = 100, units='mm', res = 300)
